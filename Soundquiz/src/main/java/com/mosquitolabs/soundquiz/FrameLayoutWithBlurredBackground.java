...
public class FrameLayoutWithBlurredBackground extends FrameLayout {
    ...
    public FrameLayoutWithBlurredBackground(Context context) {
        super(context);
        setUpBlurIntrinsic(context);
    }

    public FrameLayoutWithBlurredBackground(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        setUpStyleableAttributes(attrs);
        setUpBlurIntrinsic(context);
    }

    public FrameLayoutWithBlurredBackground(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        setUpStyleableAttributes(attrs);
    }

    private void setUpStyleableAttributes(AttributeSet attrs) {
        TypedArray a = getContext().getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.FrameLayoutWithBlurredBackground,
                0, 0);

        try {
            blurRadius = a.getFloat(R.styleable.FrameLayoutWithBlurredBackground_blurRadius,
                    DEFAULT_BLUR_RADIUS);
